...
path_queries ="/export/usuarios_ml4ds/lbartolome/Repos/umd/LinQAForge/src/qa_system/questions_annotate_tpc15_qwen:32b_full_with_queries.xlsx"
df_q = pd.read_excel(path_queries)
...
for thrs in [None, thrs_]:
    ...
    for id_row, row in tqdm(df_q.iterrows()):
        if NR_TPCS != 30:
        ...
        df_q.at[id_row, "results_1"] = pd.Series([results_1])
        df_q.at[id_row, "results_2"] = pd.Series([results_2])
        df_q.at[id_row, "results_3_weighted"] = pd.Series([results_3_weighted])
        df_q.at[id_row, "results_3_unweighted"] = pd.Series([results_3_unweighted])
        df_q.at[id_row, "results_4_weighted"] = pd.Series([results_4_weighted])
        df_q.at[id_row, "results_4_unweighted"] = pd.Series([results_4_unweighted])
    ...
    path_save = path_queries.replace(".xlsx", f"_results_model{NR_TPCS}tpc_thr_{save_thr}.xlsx")
    df_q.to_excel(path_save)
    df_q = df_q.explode("results_1").explode("results_2").explode("results_3_weighted").explode("results_3_unweighted").explode("results_4_weighted").explode("results_4_unweighted")

    # for each row (question), get all the results and store them in a single column
    def combine_results(x):
        doc_ids = []
        for col in ["results_1", "results_2", "results_3_weighted", "results_3_unweighted", "results_4_weighted", "results_4_unweighted"]:
            try:
                content_col = ast.literal_eval(x[col])
            except:
                content_col = x[col]
                
            for result_q in content_col:
                
                doc_ids += [doc["doc_id"] for doc in result_q]        
        return list(set(doc_ids))

    df_q["all_results"] = df_q.apply(lambda x: combine_results(x), axis=1)

    df_q_eval = df_q.copy()[['pass_id', 'doc_id', 'passage', 'top_k', 'question','queries', 'all_results']]

    # explode the dataframe so each row contains a single result
    df_q_eval = df_q_eval.explode("all_results")
    # get content of the documents
    df_q_eval["all_results_content"] = df_q_eval.apply( lambda x: raw.loc[raw["doc_id"] == x["all_results"], "text"].values[0] if x["all_results"] in raw["doc_id"].values else None, axis=1 )

    path_save = path_queries.replace(".xlsx", f"_results_model{NR_TPCS}tpc_thr_{save_thr}_combined_to_retrieve_relevant.xlsx")
    df_q_eval.to_excel(path_save)
