...
def display_document(doc_id, doc_content):    
    ...
    display(HTML("<b><span style='color: black;'>Please provide the label for the queried instance (a combination of 0s and 1s for each category, separated by commas):</span></b>"))
    

class MultiBlade(object):
    def __init__(
        self,
        model_path: str = None,
        lang: str = None,
    ...
    def do_update(self, label):
        ...
        self.y_train = np.vstack([self.y_train, label])
    ...
    def request_labels(self, query_instances, indices):
        ...
        for query_instance, idx in zip(query_instances, indices):
            ...
            while True:
                try:
                    label_input = input()
                    label = list(map(int, label_input.split(',')))
                    if len(label) == 4 and all(l in [0, 1] for l in label):
                        break
                    else:
                        display(HTML("<b><span style='color: red;'>Invalid input. Please enter 4 values (0 or 1) separated by commas.</span></b>"))
                except ValueError:
                    display(HTML("<b><span style='color: red;'>Invalid input. Please enter 4 values (0 or 1) separated by commas.</span></b>"))
