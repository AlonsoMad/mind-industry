...
class MIND:
    ...
    def __init__(
        ...
        if self._do_check_entailment:
            ...
            self._nli_model_name = self.config.get("nli_model_name", "potsawee/deberta-v3-large-mnli")
            try:
                from transformers import (  # type: ignore
                    AutoModelForSequenceClassification, AutoTokenizer)
                self._nli_tokenizer = AutoTokenizer.from_pretrained(self._nli_model_name, use_fast=False)
    ...
    def _check_entailement(self, textA, textB, threshold=0.5):
        ...
        try:
            ...
            with torch.no_grad():
                logits = self._nli_model(**inputs).logits  # neutral already removed (2 classes)
