"""Legacy CLI entry point — preserves the old cli.py logic for backward compatibility."""
import argparse
import json
...
def build_parser() -> argparse.ArgumentParser:
    ...
    argparser.add_argument("--src_filter_ids_path", default=None, type=str,
                           help="Path to a file with IDs to filter out from the source corpus.")
    argparser.add_argument("--previous_check", default=None, type=str,
                           help="Path to a file with IDs to filter out from the source corpus based on previous checks.")
...
def legacy_main():
    import warnings
    warnings.warn(
        "The legacy 'mind-legacy' entry point is deprecated and will be removed in a future release. "
        "Please migrate to: mind detect run --config my_run.yaml",
        DeprecationWarning,
        stacklevel=1,
    )
    ...
    from mind.pipeline.pipeline import MIND  # deferred to avoid import-time heavy deps
    mind = MIND(**cfg)
